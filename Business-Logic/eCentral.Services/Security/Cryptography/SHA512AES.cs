...
namespace eCentral.Services.Security
{
    ...
    /// </summary>
    internal sealed class SHA512AES
    {
        ...
        public static byte[] MessageSHA(byte[] message)
        {
            int n, pBase;
            long l;
            byte[] m;
        ...
        private static byte[] Hash(int n, byte[] m)
        {
            // K Constants			
            uint[] K = {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int t = 0; t < 64; t++)
                {
                    temp1 = (h + Sigma(1, e) + Func(true, e, f, g)
                             + K[t] + w[t]);
                    temp2 = (Sigma(0, a) + Func(false, a, b, c));
                    h = g;
                    g = f;
                    f = e;
                    e = (d + temp1);
                ...
                interHash[0] = ((a + interHash[0]));
                interHash[1] = ((b + interHash[1]));
                interHash[2] = ((c + interHash[2]));
                interHash[3] = ((d + interHash[3]));
                interHash[4] = ((e + interHash[4]));
                interHash[5] = ((f + interHash[5]));
                interHash[6] = ((g + interHash[6]));
                interHash[7] = ((h + interHash[7]));
            }

            // Copy Intermediate results to final hash array
            for (int i = 0; i < 8; i++)
            {
                swap2 = BitConverter.GetBytes(interHash[i]);
                for (int j = 0; j < 4; j++)
                {
                    swap[j] = swap2[3 - j];
                }
        ...
        /// <summary>
        /// Performs the SHA shifting and rotating functions.
